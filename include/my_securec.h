...
/**
 * The wrapper for snprintf for using the secure version if available.
 * (note that the function my_snprintf() already exists in mysql, so
 * we named this function my_sec_snprintf() to avoid confusion.)
 *
 * Parameters:
 *    strDest - [In/Out] Destination buffer.
 *    destMax - [In] Size of the destination buffer.
 *    count - [In] maximum number of bytes to copy into strDest
 ...
 */
inline
int my_sec_snprintf(char* strDest, size_t destMax,
                    size_t count, const char* format, ...)
{
  ...
  /* It returns the number of characters written (not including the
   * terminating NULL) on success, or -1 on errors. */
#else
...
#error "Error: Secure C library must be used!"
  ...
  rc = vsnprintf(strDest, count, format, args);
#endif
  va_end(args);
