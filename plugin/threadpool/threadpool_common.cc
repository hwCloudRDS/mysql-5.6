...
/* adopted from percona 5.6 with following changes
 * thd->event_scheduler change to thd->scheduler due to THD definition in sql/sql_class.h */
...
uint threadpool_idle_timeout; /* in seconds */
uint threadpool_prio_kickup_timer; /* in milliseconds */
...
int threadpool_add_connection(THD *thd)
{
  ...
  */
    ...
    if (!rc)
    {
      ...
      */
      if (thd_is_connection_alive(thd))
      {
        ...
        //TODO : since no such variable in thd, we comment thd->skip_wait_timeout= true;
  ...
  worker_context.restore();
  //TODO: since no such variable in thd, we comment thd->system_tid = -1;
  return retval;
}
...
/*  we don't need this in plugin 
 *  This is used by percona to hool into mysql
