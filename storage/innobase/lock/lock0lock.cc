...
#include "mysqld.h" /* for using current_thd */
	...
	ReadView*	    view)	/*!< in: consistent read view */
{
    ...
    if (srv_read_only_mode || NULL == view) {
        ut_ad(view == 0);
        return(true);
    }
	...
	const lock_t* wait_for)
{
	...
	if (victim_trx != 0) {
        ...
        /* row_search_for_mysql() calls trx_start_if_not_started() with the
        read_write parameter set to false. So for an insert sub-select statement
        a transaction may not have an ID assigned yet if the deadlock is hit
        during the select part of the procesing. As such we can't assert
        victim_trx->id > 0 */
	...
	/* Reach here: Should return DB_LOCK_WAIT */
	...
	if (lock_rec_has_expl(mode, block, heap_no, trx)) {
		...
		if (wait_for != NULL) {
			...
			err = lock_rec_enqueue_waiting(
					mode, block, heap_no, index, thr, wait_for);
...
loop:
	...
	if (nth_lock == 0) {
        ...
        // Test for a valid read view before printing
				...
				record */
{
	...
	*inherit = TRUE;
	...
	if (wait_for != NULL) {
		...
		err = lock_rec_enqueue_waiting(
			LOCK_X | LOCK_GAP | LOCK_INSERT_INTENTION,
			block, next_rec_heap_no, index, thr, wait_for);
