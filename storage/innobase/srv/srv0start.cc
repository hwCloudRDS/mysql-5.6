...
UNIV_INTERN bool	srv_is_being_started = false;
...
/*====================================*/
{
	...
	if (srv_read_only_mode) {
		ib_logf(IB_LOG_LEVEL_INFO,
                ...
                "Double-write buffer is disabled.");

        // WL#7682 (commit 810de5c)
        /* There is no write and so turn-off
...
#define BUF_POOL_SIZE_THRESHOLD (1024 * 1024 * 1024)
    ...
    if (srv_n_page_cleaners > srv_buf_pool_instances) {
    	ib_logf(IB_LOG_LEVEL_INFO,
		        "Num page cleaners (%lu) is greater than num buffer pool instances (%lu).\n"
                "Setting num page cleaners to num buffer pool instances.",
...
# endif /* __WIN__ */
    ...
    /* We preserve the 5.6 behavior of not bringing up the page cleaners
    in server read-only mode.
    Need to make sure cleaner initialization is done after recv_sys_create(),
    and also need to make sure cleaners are up before 
    buf_flush_sync_all_buf_pools() and recv_recovery_from_checkpoint_start()
    are called because with the 5.7-based page cleaner and parallel DWB
    patches all flushing is done by cleaner threads. */
    if (!srv_read_only_mode) {
        err = buf_flush_page_cleaner_init();
	...
	if (create_new_db) {
        /* page cleaner patch: wake page cleaner up to progress
        (from Bug#1880275 (commit ca25011a) & Bug#18253089 (commit 30a08cf))*/
        recv_sys_debug_free();
