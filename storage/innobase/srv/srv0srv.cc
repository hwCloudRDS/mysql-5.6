...
UNIV_INTERN ulong	srv_doublewrite_batch_size	= 64;
...
UNIV_INTERN my_bool     srv_cleaner_real_time   = TRUE;
...
/*==========*/
{
    ...
    /* Page cleaners come up even in read-only mode 
...
/*==========*/
{
    ...
    // WL#7682 (commit 810de5c)
    /* Page cleaners come up even in read-only mode 
    and so we need to free the mutex. */
	//if (!srv_read_only_mode) 
	if (!srv_read_only_mode) {
