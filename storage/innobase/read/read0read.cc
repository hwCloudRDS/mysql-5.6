...
MVCC::validate() const
{
    ...
    //ut_list_map(m_views, &MVCC::m_views, ViewCheck());
    ut_list_map(m_views, &ReadView::m_view_list, ViewCheck());
    ...
    if (p != NULL) {
        ...
        if (NULL == ret_p) {
           ib_logf(IB_LOG_LEVEL_ERROR,
                   "memmove violation: read0read.cc:%d", __LINE__);
        }
        ut_ad(ret_p != NULL);
...
MVCC::MVCC(ulint size)
{
    ...
    for (ulint i = 0; i < size; ++i) {
        ...
        UT_LIST_ADD_FIRST(m_view_list, m_free, view);
    }
}
...
MVCC::~MVCC()
{
    for (ReadView* view = UT_LIST_GET_FIRST(m_free);
        ...
        UT_LIST_REMOVE(m_view_list, m_free, view);
...
MVCC::view_release(ReadView*& view)
{
    ...
    UT_LIST_REMOVE(m_view_list, m_views, view);

    UT_LIST_ADD_LAST(m_view_list, m_free, view);
...
MVCC::view_open(ReadView*& view, trx_t* trx)
{
    ...
    if (view != NULL) {
        ...
        UT_LIST_ADD_FIRST(m_view_list, m_views, view);
