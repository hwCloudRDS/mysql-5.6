	...
	mtr_t*	mtr)	/*!< in/out: mtr */
{
    ...
    // We want the while log buffer size, not just a slot size
	data_size = dyn_array_get_data_size(mlog);

    if (data_size > log_sys->buf_size / 2) {
		...
		ib_logf(IB_LOG_LEVEL_WARN,
			...
			"Trying to extend it.",
			data_size, LOG_BUFFER_SIZE);

        log_buffer_extend((data_size + 1) * 2); 
    }

    /* enhanced adaptive flushing patch.
    Do a log margin check here (with log mutex protection).
    To minimize code re-structure we will get the log mutex
    and release it after the margin check. */
    mutex_enter(&log_sys->mutex);

    log_margin_checkpoint_age(data_size);
    ...
    // data_size shouldn't have changed from what we've assigned above
    ut_ad(data_size == dyn_array_get_data_size(mlog));

	/* Open the database log for log_write_low */
    // Don't try to extend log buffer again since we've just done it above
	mtr->start_lsn = log_reserve_and_open(data_size, false);
