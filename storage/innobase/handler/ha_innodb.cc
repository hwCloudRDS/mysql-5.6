...
#  endif /* UNIV_MEM_DEBUG */
    ...
    {&page_cleaner_mutex_key, "page_cleaner_mutex", 0},
...
static PSI_thread_info	all_innodb_threads[] = {
	...
	{&page_cleaner_thread_key, "page_cleaner_thread", 0},
...
static PSI_file_info	all_innodb_files[] = {
	...
	{&innodb_file_parallel_dblwrite_key, "innodb_parallel_dblwrite_file", 0}
};
# endif /* UNIV_PFS_IO */
...
/*================*/
	handlerton*	hton,		  /*!< in: InnoDB handlerton */
    bool binlog_group_flush); /*!< in: true if invoked by binlog group commit */
						...
						from check function */
{
    ...
    /* BUG#20511314 - need to validate tablespace id first.
    We test whether the tablespace exists or not by trying
    to get its version number (-1 is returned if not found). */
    ib_int64_t  space_ver = fil_space_get_version(space_id);
    ...
    /* BUG#22311319 - need to check the tablespace size too. */
    space_size = fil_space_get_size(space_id);
    if (srv_saved_page_number_debug > space_size) {
		ib_logf(IB_LOG_LEVEL_WARN,
                "innodb_make_page_dirty(): "
                "innodb_saved_page_number_debug > tablespace size; "
                "innodb_saved_page_number_debug = %lu; "
                "tablespace (id %lu) size = %lu",
                srv_saved_page_number_debug,
                space_id,
                space_size);
        return;
						...
						check function */
{
    if (srv_read_only_mode) {
		ib_logf(IB_LOG_LEVEL_ERROR,
                "Server debug variable innodb_log_checkpoint_now cannot be "
						...
						check function */
{
    if (srv_read_only_mode) {
		ib_logf(IB_LOG_LEVEL_ERROR,
                "Server debug variable innodb_buf_flush_list_now cannot be "
                "set when InnoDB is in read-only mode. This is a no-op.");
        return;
    }

	if (*(my_bool*) save) {
        // From commit 1f4f1353 in 5.7.9 (Bug#20582189)
