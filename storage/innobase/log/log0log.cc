    ...
    if (margin > log_sys->log_group_capacity) {
        ...
        if (!log_has_printed_chkp_margine_warning
		    ...
		    ib_logf(IB_LOG_LEVEL_ERROR,
                    "The transaction log files are too"
                    " small for the single transaction log (size=%lu"
                    "). So, the last checkpoint age"
                    " might exceed the log group capacity %lu.",
                    len,
...
/*=================*/
    ...
    bool check_log_extension) /*!< in: whether to check for log
                                extension (default=true) */
{
	...
	if (check_log_extension && len >= log->buf_size / 2) {
...
UNIV_INTERN
void
log_buffer_flush_to_disk(bool sync)
/*==========================*/
{
	lsn_t	lsn;
				...
				at least to this lsn */
{
    ...
    /* With the Parallel DWB patch, pre-flush is now always
    done by page cleaner threads, even for the
    new_oldest = LSN_MAX case. */
    if (srv_flush_sync) {
