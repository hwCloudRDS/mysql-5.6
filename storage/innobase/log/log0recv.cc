...
UNIV_INTERN volatile ibool	recv_recovery_on;
...
UNIV_INTERN volatile bool		recv_writer_thread_active = false;
...
/*================*/
{
	if (recv_sys != NULL) {
        ...
        if (recv_sys->flush_start != NULL) {
            os_event_free(recv_sys->flush_start);
        }

        if (recv_sys->flush_end != NULL) {
            os_event_free(recv_sys->flush_end);
        }
...
# define LIMIT_LSN		LSN_MAX
    ...
    /* Parallel DWB patch:
    recv_log_format_0_recover() logic is not in 5.6. --ie,
    in 5.6 there is no concept of redo log versioning and
    recovery from previous log version is not applicable. */
    ...
    if (!recv_needed_recovery) { 
        ...
        ib_logf(IB_LOG_LEVEL_INFO,
                "recv_recovery_from_checkpoint_start_func(): finished DWB recovery "
                "and deleted DWB file.");
    }
