...
/*============================*/
{
	...
	for (i = 0; i < TRX_SYS_N_RSEGS; ++i) {
		...
		for (undo = UT_LIST_GET_FIRST(rseg->insert_undo_list);
            ...
            // Need to insert trx in rw_trx_list in descending order
		...
		for (undo = UT_LIST_GET_FIRST(rseg->update_undo_list);
		     undo != NULL;
		     undo = UT_LIST_GET_NEXT(undo_list, undo)) {
            bool trx_created = false;

			/*
              Check the trx_sys->rw_trx_list first, since the trx
              could have been in insert_undo_list too and was added
              to rw_trx_list already in the logic above.
            */
			mutex_enter(&trx_sys->mutex);
    ...
    // During recovery rw_trx_ids needs to be in ascending order
    // for trx_commit_low (at least for silencing an assertion).
    trx_t* trx;
    for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list);
        ...
        if (trx->state == TRX_STATE_ACTIVE ||
            trx->state == TRX_STATE_PREPARED) {
            trx_sys->rw_trx_ids.insert(trx_sys->rw_trx_ids.begin(),
                                       trx->id);
        }
    }
}
    ...
    bool    read_write) /*!< in: true if read-write transaction */
{
    ...
    /* Note: Transactions marked explicitly as read-only can write to
    temporary tables and we put them on the RW list and assign a rollback
    segment to it too. */
    ...
    if (!trx->read_only
        ...
        if (!trx_is_autocommit_non_locking(trx)) {
            ...
            to write to the temporary table, and we need to assign
            it a rollback segment and add it to rw_trx_list. */
            if (read_write) {
                ...
                // Need to set the state before trx_sys_validate_trx_list()
                trx->state = TRX_STATE_ACTIVE;
    ...
    // the following code is from WL#6578 but i've commented
    // it out because thd_start_time_in_secs() actually calls ut_time(),
    // so there's no need to pay the cost of the "if" statement and
	...
	trx_t*		trx)	/*!< in: transaction */
{
    ...
    if (!trx->read_only) {
        UT_LIST_ADD_LAST(no_list, trx_sys->serialisation_list, trx);
        added_trx_no = true;
    } else {
        added_trx_no = false;
    }
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	if (trx->update_undo != NULL) {
		...
		serialised = trx_serialisation_number_get(trx);
			...
			if the transaction did not modify anything */
{
	...
	if (trx_is_autocommit_non_locking(trx)) {
        ...
        // We can't use the 5.7 check of (trx->read_only || trx->rseg == 0)
        // because we don't have the rw_trx_set mapping structure (due to
        // the perf degradation it caused in update workloads) and so
        // a read-only trx (ie, read_only is true) that did internal table
        // update would have been added to rw_trx_list with in_rw_trx_list=true
        // and so we would need to do the necessary cleanup.
        if (trx->id == 0) {
            ut_ad(!trx->in_rw_trx_list);
            ut_a(trx->rseg == 0);
    ...
    // Added as part of WL#6578 port (trx_init() equivalent changes)
    ut_ad(!MVCC::is_view_active(trx->read_view));
			...
			or NULL if trx made no modifications */
{
    ...
    if (trx->id > 0) {
        ...
        if (serialised) {
            UT_LIST_REMOVE(no_list, trx_sys->serialisation_list, trx);
        }
	...
	trx_t*	trx)	/*!< in/out: active transaction */
{
    ...
    // Note that mvcc->view_open(...) already updates creator_trx_id
    // with trx->id.
    if (srv_read_only_mode) {
    ...
    trx_t*      trx)        /*!< in/out: transaction that is RW */
{
    ...
    add the trx to the rw list. If in future, some other thread
    ...
    // even for a read only transaction we need to add
    // it to the rw_trx_list since we don't have the rw_trx_set
    // mapping structure (from 5.7).
    UT_LIST_ADD_FIRST(trx_list, trx_sys->rw_trx_list, trx);
