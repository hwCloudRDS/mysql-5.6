	...
	const trx_t*	in_trx)	/*!< in: transaction */
{
    ...
    for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list);
        ...
        ut_ad(trx->rseg != NULL && !trx->read_only);
	}
...
/*===============*/
{
    ...
    /* Note that we still need to call buf_parallel_dblwr_free() even if 
    srv_use_doublewrite_buf is FALSE in order to free parallel_dblwr_buf.path
    buffer. The path buffer is constructed because crash recovery can use the
    double write buffer left over from previous crash even when mysqld is
    started with srv_use_doublewrite_buf FALSE. */ 
