...
void Binlog_sender::init()
{
  ...
  if (m_packet.realloc(PACKET_MIN_SIZE)) {
...
int Binlog_sender::check_start_file()
{
  ...
  else if (m_using_gtid_protocol)
  {
    ...
    */
    ...
    Gtid_set
      gtid_executed_and_owned(gtid_state->get_logged_gtids()->get_sid_map());

    // gtids = executed_gtids & owned_gtids
    if (gtid_executed_and_owned.add_gtid_set(gtid_state->get_logged_gtids())
        != RETURN_STATUS_OK)
    {
      DBUG_ASSERT(0);
    }
                                     ...
                                     my_off_t log_pos)
{
  ...
  */
  ...
  size_t max_size= (size_t) event_len
                    - LOG_EVENT_HEADER_LEN - ROTATE_HEADER_LEN;
  if (my_memcpy(rotate_header + ROTATE_HEADER_LEN, 
  ...
  if (m_using_gtid_protocol)
  {
    if (m_gtid_clear_fd_created_flag)
    {
      ...
      */
      ...
      /* We only do the above for the first_gtid 
       * - Not see problems without it for now - comment it out */
      // m_gtid_clear_fd_created_flag = FALSE;
    }
  }
...
inline uint32 Binlog_sender::calc_grow_buffer_size(uint32 current_size,
		                                           uint32 min_size)
{
   ...
   */
  uint32 new_size= static_cast<size_t>(
...
void Binlog_sender::calc_shrink_buffer_size(uint32 current_size)
{
  uint32 new_size= static_cast<size_t>(
