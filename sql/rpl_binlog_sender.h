...
public:
  Binlog_sender(THD *thd, const char *start_file, my_off_t start_pos,
              const Gtid_set *exclude_gtids, uint32 flag)
    ...
    m_errmsg(NULL), m_errno(0), m_last_file(NULL), m_last_pos(0),m_flag(flag),
	m_observe_transmission(false), m_transmit_started(false),
...
private:
  ...
  */
  inline uint32 calc_grow_buffer_size(uint32 current_size, uint32 min_size);
  ...
  /**
   ...
   @param current_size The baseline (for instance, the current buffer size).
  */
  void calc_shrink_buffer_size(uint32 current_size);
};
