                                                      ...
                                                      bool need_lock)
{
  ...
  */
  ...
  {
    my_atomic_store64_tmp(&last_lwm_timestamp, lwm_estim);
  }
                            ...
                            longlong lwm_estimate_arg)
{
  ...
  my_atomic_store64_tmp(&min_waited_timestamp, last_committed_arg);
  /*
  ...
  */
      ...
      !clock_leq(last_committed_arg, get_lwm_timestamp(rli, true)))
  {
    ...
    my_atomic_store64_tmp(&min_waited_timestamp, SEQ_UNINIT);  // reset waiting flag
    /* Note: In 5.6 the following EXIT_COND call unlocked rli->mts_gaq_LOCK
     *       - In 5.7 need unlock it here: 5.7 EXIT_COND does not unlock it inside */
    thd->EXIT_COND(&old_stage);
    mysql_mutex_assert_not_owner(&rli->mts_gaq_LOCK);
    set_timespec_nsec(ts[1], 0);
    my_atomic_add64_tmp(&rli->mts_total_wait_overlap, diff_timespec(ts[1], ts[0]));
  }
  else
  {
    my_atomic_store64_tmp(&min_waited_timestamp, SEQ_UNINIT);
    mysql_mutex_unlock(&rli->mts_gaq_LOCK);
  }
  ...
  else
  {
    ...
    */
    if (sequence_number == SEQ_UNINIT && last_committed == SEQ_UNINIT)
      rli->last_assigned_worker= *(Slave_worker**) dynamic_array_ptr(&rli->workers, 0);;
  }
