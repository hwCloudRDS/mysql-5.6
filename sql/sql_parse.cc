  ...
  */
  ...
  case SQLCOM_RESET:
    ...
    */
	...
	if(!is_rds_reserved_user(thd->security_ctx->user) && check_global_access_noerr(thd,SUPER_ACL))
    {
...
create_sp_error:
  ...
  case SQLCOM_ALTER_TABLE:
  {
	LEX *lex= thd->lex;
	SELECT_LEX *select_lex= &lex->select_lex;
	if(opt_rds_permission_control && check_mysql_db_tables(thd, (TABLE_LIST*) select_lex->table_list.first, FALSE, FALSE))
	{
	  res= TRUE;
	  break;
	}
	...
	if(opt_rds_permission_control &&
	   ...
	   check_performance_schema_db_tables(thd, (TABLE_LIST*) select_lex->table_list.first, FALSE, FALSE))
	{
	  res= TRUE;
	  break;
	}
