...
/**
   ...
   @see grant_init()
   @note The LOCK_grant is partitioned into an array of rw_locks in hwsql.
   @see Partitioned_rwlock in Partitioned_rwlock.h
 */
...
a file name for --log-bin-index option", opt_binlog_index_name);
  ...
  */
  ...
  if (!DEFAULT_ERRMSGS[0][0]) {
    sql_print_error("The error messages file \"errmsg.sys\" is not loaded! "
                    "Check the \"lc_messages_dir\"setting. ");
    unireg_abort(1);
  }
  ...
  */
  ...
  if (!opt_bootstrap && !opt_noacl)
  {
    ...
    handlerton *default_se_handle=
      plugin_data(global_system_variables.table_plugin, handlerton*);
    ...
    handlerton *default_tmp_se_handle=
      plugin_data(global_system_variables.temp_table_plugin, handlerton*);
