...
bool gtid_stmt_allowed(const THD *thd)
{
  /*
...
public:
  ...
  */
  ...
  int flush(THD *thd, my_off_t *bytes_written, bool *wrote_xid)
  {
    ...
    {
      DBUG_ASSERT(gtid_stmt_allowed(thd));
      /*
...
binlog_cache_data::flush(THD *thd, my_off_t *bytes_written, bool *wrote_xid)
{
  ...
  */
  ...
  if (flags.finalized)
  {
    ...
    thd->transaction.sequence_number= mysql_bin_log.transaction_counter.step();
    /*
      ...
      commit parent).
    */
    if (thd->transaction.last_committed == SEQ_UNINIT)
      thd->transaction.last_committed= thd->transaction.sequence_number - 1;
...
static int binlog_prepare(handlerton *hton, THD *thd, bool all)
{
  ...
  if (!all)
  {
    ...
    thd->transaction.store_commit_parent(clock.get_timestamp());
  }
  DBUG_RETURN(0);
}
                                   ...
                                   bool need_engine_log_flush)
{
  ...
  /*
    ...
    LSN to flush to), even f their LSNs are larger than this thread's.
  ...
  *slot= m_first->stage_cond_id;
  ...
  /*
    ...
    the queue as well.
  */
  m_size = 1;  // We locked the queue so this is safe
                                                 ...
                                                 bool *need_engine_log_flush)
{
  ...
  DBUG_PRINT("info", ("m_first: 0x%llx, &m_first: 0x%llx, m_last: 0x%llx",
                       (ulonglong) m_first, (ulonglong) &m_first,
                       (ulonglong) m_last));
  DBUG_PRINT("info", ("fetched queue of %d transactions", m_size));
  DBUG_ASSERT(m_first || m_last == &m_first);
  DBUG_PRINT("return", ("result: 0x%llx", (ulonglong) result));
  DBUG_ASSERT(m_size >= 0);

  m_size = 0;  // We locked the queue so this is safe.
...
int MYSQL_BIN_LOG::rollback(THD *thd, bool all)
{
  ...
  */
  ...
  if (stuff_logged)
  {
    thd->transaction.store_commit_parent(max_committed_transaction.get_timestamp());
  }
...
int MYSQL_BIN_LOG::new_file_impl(bool need_lock_log, Format_description_log_event *extra_description_event)
{
  ...
  */
  if (flush_io_cache(&log_file)) {
    sql_print_error("Failed to flush I/O cache on log (file '%s')",
                    log_file_name);;
    error= 1;
    close_on_error= TRUE;
    goto end;
    ...
    if (thd)
    {
      if (!thd->is_current_stmt_binlog_format_row())
      {
        ...
        if (thd->user_var_events.elements)
        {
          ...
          {
            BINLOG_USER_VAR_EVENT *user_var_event=NULL;
  ...
  if (likely(is_open()))                       // Should always be true
  {
    ...
    if (my_b_tell(cache) > 0)
    {
      ...
      */
      ...
      DBUG_ASSERT(thd->transaction.sequence_number > clock.get_offset());

      int64 seq_number_logged= thd->transaction.sequence_number - clock.get_offset();
      int64 last_committed_logged=
          thd->transaction.last_committed <= clock.get_offset() ?
              SEQ_UNINIT : thd->transaction.last_committed - clock.get_offset();
      /*
      ...
      */
      ...
      thd->transaction.last_committed= SEQ_UNINIT;

      DBUG_EXECUTE_IF("crash_before_writing_xid",
                      {
                        if ((write_error= do_write_cache(cache, last_committed_logged,
                                                         seq_number_logged)))
...
{
  max_committed_transaction.set_if_greater(thd->transaction.sequence_number);
  /*
    sequence_number timestamp is unneeded anymore, so it's cleared off.
  */
  thd->transaction.sequence_number= SEQ_UNINIT;

  DBUG_ASSERT(thd->transaction.last_committed == SEQ_UNINIT ||
              thd->commit_error == THD::CE_FLUSH_ERROR);
}
...
#endif
    if (head->transaction.sequence_number != SEQ_UNINIT)
...
MYSQL_BIN_LOG::finish_commit(THD *thd)
{
  ...
  */
  ...
  if (thd->transaction.sequence_number != SEQ_UNINIT)
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit)
{
  ...
  */
  /*
    ...
    Note: Not TRUE after now: the dump thread do not acquire LOCK_log when reading.
  */
  if (change_stage(thd, Stage_manager::SYNC_STAGE, stage_queue,
                   &LOCK_log, &LOCK_sync))
  {
    DBUG_PRINT("return", ("Thread ID: %lu, commit_error: %d",
                             thd->thread_id, thd->commit_error));
...
int64 Logical_clock::get_timestamp()
{
  ...
  retval= my_atomic_load64_tmp(&state);
  DBUG_RETURN(retval);
}
...
inline int64 Logical_clock::set_if_greater(int64 new_val)
{
  ...
  while (!(cas_rc= my_atomic_cas64_tmp(&state, &old_val, new_val)) &&
         old_val < new_val)
  {}
