...
int init_slave()
{
  ...
  */
  ...
  /* Check slave config conflicts */
  if (check_slave_sql_config_conflict(NULL, NULL))
  {
    error= 1;
    goto err;
  }
...
int slave_start_single_worker(Relay_log_info *rli, ulong i)
{
  ...
  //w->currently_executing_gtid.clear(); /* CRC: we need 3656 */
  ...
  */
  ...
  for (i= rli->workers.elements - 1; i >= 0; i--)
  {
    ...
    while (w->running_status != Slave_worker::NOT_RUNNING)
    {
      ...
      mysql_cond_wait(&w->jobs_cond, &w->jobs_lock);
      thd->EXIT_COND(&old_stage);
      mysql_mutex_lock(&w->jobs_lock); // re-acquire it as exit_cond() released
    }
    mysql_mutex_unlock(&w->jobs_lock);
  }
...
pthread_handler_t handle_slave_sql(void *arg)
{
  ...
  //mysql_mutex_lock(&rli->info_thd_lock); // May crash without it?
  rli->info_thd= thd;
  ...
  */
  ...
  while (!sql_slave_killed(thd,rli))
  {
...
#ifndef DBUG_OFF
    /* This is for the test case rpl.sql_thread_error:
     * - It need the following debug instrument */
    ...
    */
    ...
    bool can_read_event = false;

    if(!hot_log ||
      rli->get_event_relay_log_pos() < relaylog_end_pos)
    {
      can_read_event = true;
    }
	else
	{
      ...
      if (0 == flush_io_cache(rli->relay_log.get_log_file()))
        { relaylog_end_pos = rli->relay_log.get_log_file()->end_of_file; }
      mysql_mutex_unlock(log_lock);
      if (!rli->relay_log.is_active(rli->get_event_relay_log_name()) ||
        rli->get_event_relay_log_pos() < relaylog_end_pos)
      {
        can_read_event = true;
      }
    ...
    if (can_read_event && (ev= Log_event::read_log_event(cur_log, 0,
    ...
    if (!cur_log->error) /* EOF */
    {
      ...
      */
      ...
      {
        mysql_mutex_lock(log_lock);
          if (rli->relay_log.get_log_file()->end_of_file > rli->get_event_relay_log_pos() ||
...
bool change_master(THD* thd, Master_info* mi)
{
  ...
  */
  ...
  if (lex_mi->relay_log_name)
  {
    ...
    mi->rli->is_group_master_log_pos_invalid= true; /* BUG#11766010 Fix */
  }
