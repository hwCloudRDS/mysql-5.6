...
#define GOTO_UPDATE_ERROR(A) { error = A; goto err; }
    ...
    origin_ptr= (char *) my_malloc(len + 1, MYF(0));
    if (!origin_ptr)
    {
...
/**
  Check if performance_schema db
*/
...
int check_reserved_user(THD *thd, List <LEX_USER> &list, const char *command)
{
  ...
  if(NULL != thd->security_ctx->user && is_rds_reserved_user(thd->security_ctx->user) )
  {
    DBUG_RETURN(0);
  }
...
int check_internal_db(THD *thd, const char *db)
{
  ...
  if(is_rds_reserved_user(thd->security_ctx->user))
  {
    DBUG_RETURN(0);
  }
  ...
  my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), thd->security_ctx->user, thd->security_ctx->host_or_ip, db);
  DBUG_RETURN(1);
}
...
int check_mysql_db_tables(THD *thd, TABLE_LIST *tables,bool do_check_one_table,bool no_error)
{
  ...
  for (; tables != first_not_own_table && tables; tables= tables->next_global)
  {
    if (is_mysql_db(tables->get_db_name()))
	{
	  if (!no_error)
       {
		my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), thd->security_ctx->user, thd->security_ctx->host_or_ip, tables->get_db_name());
	   }
	   DBUG_RETURN(1);
	}
  }
