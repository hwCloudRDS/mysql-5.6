  ...
  */
  if (buf == local_buf)
  {
    ...
    if (new_alloc_len > packet->alloced_length() &&
        packet->realloc(new_alloc_len, false))
    {
        ...
        worker_id != MTS_WORKER_UNDEF)))
  {
    if (!rli->curr_group_seen_gtid && !rli->curr_group_seen_begin)
    {
      ...
      if (is_s_event || is_gtid_event(this))
      {
        ...
        if (starts_group()) /* Begin statement */
        {
  ...
  else if (contains_partition_info(rli->mts_end_group_sets_max_dbs))
  {
    ...
    */
    ...
    if (mts_dbs.num == OVER_MAX_DBS_IN_EVENT_MTS)
    {
      ...
      (void)rli->current_mts_submode->wait_for_workers_to_finish(rli, ret_worker);
      /*
...
code_name(int code)
{
  ...
  case Q_COMMIT_TS2: return "Q_COMMIT_TS2";
  }
...
bool slave_execute_deferred_events(THD *thd)
{
  ...
  rli->deferred_events->rewind(); /* BUG#15850951: DUPLICATE ERROR IN REPLICATION WITH SLAVE */
  return res;
}
...
#ifndef MYSQL_CLIENT
  if (my_memset(&m_cols, sizeof(m_cols), 0, sizeof(m_cols),
                sql_print_error) == NULL) {
    /* Bug#15829568 Fix */
    /* This should not happen and it is in constructor! */
    sql_print_error("Failed to initialize columns bitmap object (size %zu)",
                    sizeof(m_cols));
    DBUG_VOID_RETURN; /* Just like other error failure in this function */
  }
#else
  if (my_memset(&m_cols, sizeof(m_cols), 0, sizeof(m_cols), NULL) == NULL) {
    /* Bug#15829568 Fix  */
    /* This should not happen and it is in constructor! */
    DBUG_VOID_RETURN; /* Just like other error failure in this function */
  }
    ...
    */
    ...
    for (const char* pos= start; pos < end;)
    {
      ...
      case RW_V_EXTRAINFO_TAG:
      {
        ...
        if ((end - pos) < EXTRA_ROW_INFO_HDR_BYTES) /* Bug#15829568 Fix */
          DBUG_VOID_RETURN;

        uint8 infoLen= pos[EXTRA_ROW_INFO_LEN_OFFSET];
        // assert((end - pos) >= infoLen);
        if ((end - pos) < infoLen) /* Bug#15829568 Fix */
          DBUG_VOID_RETURN;
