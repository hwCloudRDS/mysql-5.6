...
Relay_log_info::Relay_log_info(bool is_slave_recovery
...
#endif
                               , uint param_id,
                               bool is_rli_fake /* Bug#14678248 Fix */
                              )
   :Rpl_info("SQL"
#ifdef HAVE_PSI_INTERFACE
   ...
   group_relay_log_pos(0), event_relay_log_number(0), event_relay_log_pos(0),
   event_start_pos(0),
   group_master_log_pos(0),
   gtid_set(global_sid_map, global_sid_lock),
   rli_fake(is_rli_fake), /* Bug#14678248 Fix */
   is_group_master_log_pos_invalid(false), /* BUG#11766010 Fix */
   ...
   long_find_row_note_printed(false)
{
  ...
  mysql_mutex_init(key_mts_gaq_LOCK, &mts_gaq_LOCK,
                   MY_MUTEX_INIT_FAST);
  mysql_cond_init(key_cond_mts_gaq, &logical_clock_cond, NULL);
                                    ...
                                    double timeout)
{
  ...
  */
         ...
         slave_running)
  {
    ...
    */
    if (*group_master_log_name && !is_group_master_log_pos_invalid) /* BUG#11766010 Fix */
    {
                                             ...
                                             char name[FN_REFLEN+1])
{
  ...
  if (strLen >= (FN_REFLEN - 1)) {
    sql_print_error("Failed to make relay log name with base name "
                    "%s - relay log base name is too long " 
                    "(length %zu)", relay_log_basename, strLen);
    rc = 1;
  } else {
    ...
    *str++= '.'; 
    ...
    if (my_sprintf(str, destMax, "%06u", number) < 0) {
      sql_print_error("Failed to make relay log name with base name " 
                      ...
                      "for the number: %zu",
                      relay_log_basename, number, (destMax - 1));
      rc = 1;
    }
  }
  return rc;
